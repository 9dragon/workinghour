...
|start_time|DATETIME|-|是|-|✅|工作开始时间，用于工时核对和查询|
|end_time|DATETIME|-|是|-|-|工作结束时间，必须≥开始时间（应用层校验）|
...
class WorkHourDataModel(BaseModel):
    ...
    approval_result: str = Field(default='通过', regex='^(通过)$')
    approval_status: str = Field(default='已完成', regex='^(已完成)$')
...
```json
{
  "errors": [
    ...
    {"row": 18, "field": "审批结果", "error": "审批状态为'拒绝'，不符合导入要求"},
    {"row": 25, "field": "姓名", "error": "姓名字段为空"}
  ],
  "total_errors": 4
}
```
