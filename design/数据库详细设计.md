...
|holidays|节假日表|50+|辅助|
|employees|员工表|200+|核心|
|projects|项目表|500+|重要|
|project_budgets|项目预算表|1500+|重要|
...
CREATE TABLE employees (
    ...
    dept_name TEXT NOT NULL,                           -- 部门名称
    role TEXT NOT NULL DEFAULT 'staff',                -- 角色：project_manager/data_collection/software_dev/staff
    created_at DATETIME NOT NULL DEFAULT (datetime('now', 'localtime')),
...
CREATE INDEX idx_employees_dept_name ON employees(dept_name);
...
|employee_name|TEXT|NOT NULL|-|员工姓名（关联work_hour_data.user_name）|
|dept_name|TEXT|NOT NULL|-|部门名称|
|role|TEXT|NOT NULL|'staff'|角色分类，用于预算统计|
|created_at|DATETIME|NOT NULL|当前时间|记录创建时间|
|updated_at|DATETIME|NOT NULL|当前时间|记录更新时间|

**角色枚举值说明：**

|角色值|中文名称|说明|
|---|---|---|
|project_manager|项目管理|项目经理角色|
|data_collection|数采实施|数据采集实施角色|
|software_dev|软件实施|软件开发实施角色|
|staff|普通员工|其他员工（不纳入预算统计）|
...
CREATE TABLE projects (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    project_code TEXT NOT NULL UNIQUE,                 -- 项目代码
    project_name TEXT NOT NULL,                        -- 项目名称
    project_type TEXT NOT NULL DEFAULT 'delivery',     -- 项目类型：delivery/research
    project_type_label TEXT NOT NULL DEFAULT '项目交付', -- 项目类型标签
    project_prefix TEXT NOT NULL DEFAULT 'D',          -- 项目前缀
    project_manager TEXT,                              -- 项目经理
    status TEXT NOT NULL DEFAULT 'active',             -- 状态：active/completed
    status_label TEXT NOT NULL DEFAULT '进行中',       -- 状态标签
    created_at DATETIME NOT NULL DEFAULT (datetime('now', 'localtime')),
    updated_at DATETIME NOT NULL DEFAULT (datetime('now', 'localtime'))
);
...
|---|---|---|---|---|
|id|INTEGER|PRIMARY KEY|自增|主键|
|project_code|TEXT|NOT NULL, UNIQUE|-|项目代码，唯一标识|
|project_name|TEXT|NOT NULL|-|项目名称|
|project_type|TEXT|NOT NULL|'delivery'|项目类型：delivery(项目交付)/research(产研项目)|
|project_type_label|TEXT|NOT NULL|'项目交付'|项目类型中文标签|
|project_prefix|TEXT|NOT NULL|'D'|项目代码前缀（D或P）|
|project_manager|TEXT|-|-|项目经理姓名|
|status|TEXT|NOT NULL|'active'|项目状态：active(进行中)/completed(已完成)|
|status_label|TEXT|NOT NULL|'进行中'|状态中文标签|
|created_at|DATETIME|NOT NULL|当前时间|记录创建时间|
|updated_at|DATETIME|NOT NULL|当前时间|记录更新时间|

---

### 3.7 项目预算表（project_budgets）

#### 3.7.1 表结构

存储项目工时预算数据，按项目和预算类型分别录入。

```sql
CREATE TABLE project_budgets (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    project_name TEXT NOT NULL,                        -- 项目名称（冗余字段，便于显示）
    project_code TEXT NOT NULL,                        -- 项目代码（关联projects表）
    budget_type TEXT NOT NULL,                         -- 预算类型：project_manager/data_collection/software_dev
    budget_hours NUMERIC(10, 2) NOT NULL DEFAULT 0,    -- 预算工时（单位：人天）
    created_at DATETIME NOT NULL DEFAULT (datetime('now', 'localtime')),
    updated_at DATETIME NOT NULL DEFAULT (datetime('now', 'localtime')),
...
-- 创建索引
CREATE INDEX idx_project_budgets_project_code ON project_budgets(project_code);
CREATE INDEX idx_project_budgets_budget_type ON project_budgets(budget_type);
```
...
|---|---|---|---|---|
|id|INTEGER|PRIMARY KEY|自增|主键|
|project_name|TEXT|NOT NULL|-|项目名称（冗余字段，便于显示，关联projects.project_name）|
|project_code|TEXT|NOT NULL|-|项目代码（关联projects.project_code）|
|budget_type|TEXT|NOT NULL|-|预算类型，对应员工的role分类|
|budget_hours|NUMERIC(10, 2)|NOT NULL|0|预算工时，单位：人天|
|created_at|DATETIME|NOT NULL|当前时间|记录创建时间|
|updated_at|DATETIME|NOT NULL|当前时间|记录更新时间|
...
|类型值|中文名称|对应员工角色|
|---|---|---|
|project_manager|项目管理|project_manager|
|data_collection|数采实施|data_collection|
|software_dev|软件实施|software_dev|

#### 3.7.3 业务规则

1. **唯一性约束**：同一项目代码 + 同一预算类型只能存在一条记录
2. **预算工时**：必须为非负数，单位为人天
3. **关联约束**：project_code必须存在于projects表中（应用层校验）
